...
├── REQUIREMENTS.md                                  # Generated by /1_requirements
├── src/
│   ├── main/
...
- Use `@Autowired` constructor injection (preferred) or field injection for Alfresco services
...
- Prefer Java `@Configuration` classes for new code (Maven In-Process SDK)
- Use XML context files for Alfresco-specific bootstrap beans (DictionaryBootstrap, etc.)
- Module context (`module-context.xml`) imports sub-context files, does not define beans directly
- Out-of-Process SDK: use standard Spring Boot `@Configuration` and `@EventListener` / `@SpringBootApplication`
...
### Authentication
- Use ticket-based authentication or OAuth2 for production
